...
export class StatusBar extends BaseComponent<Props, State>{
        ...
        const updateRendered =
            ...
            && <span style={csx.extend(styles.statusBarSection) }>
                <span
                    ...
                    data-hint={ `Update ${serverState.update.latest} available (current: ${serverState.update.current})` + ". Please run `npm i -g spork`"}>
                ...
                <div style={csx.extend(styles.statusBar,csx.horizontal,csx.center, styles.noWrap)}>
                    ...
                    <span style={csx.extend(styles.statusBarSection)}>
                        {this.props.socketConnected?
                             <span className="hint--left hint--success" data-hint="Connected to server"> <Icon style={{color:styles.successColor, cursor:'pointer'}} name="flash" onClick={()=>ui.notifySuccessNormalDisappear("Connected to spork server")}/></span>
                            :<span className="hint--left hint--error" data-hint="Disconnected from server"> <Icon style={{color:styles.errorColor, cursor:'pointer'}} name="spinner" spin={true} onClick={()=>ui.notifyWarningNormalDisappear("Disconneted from spork server")}/></span>}
                    </span>
                    <span style={csx.extend(styles.statusBarSection, styles.noSelect, styles.hand)}>
    ...
    giveStar = () => {
        window.open('https://github.com/johnpaularthur/spork')
    }
    ...
    whatsNew = () => {
        ...
        window.open(`https://github.com/johnpaularthur/spork/compare/v${update.current}...v${update.latest}`);
    }
}
